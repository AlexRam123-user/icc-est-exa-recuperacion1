...
public class App {
    public static void main(String[] args) throws Exception {
        ...
        for (int edad : edadesBuscadas) {
            int posicion = controller.buscarPorEdad(personas, edad);
            if (posicion != -1) {
                System.out.println("Edad " + edad + " encontrada en posición " + posicion + ": " +
                        personas[posicion].nombre + ", " + personas[posicion].edad + " años.");
            } else {
        ...
        controller.ordenarPorNombre(personas);
        imprimirPersonas(personas);

        String[] nombresBuscados = { "Anais", "Miguel" };
        for (String nombre : nombresBuscados) {
            int posicion = controller.buscarPorNombre(personas, nombre);
            if (posicion != -1) {
                System.out.println("Nombre " + nombre + " encontrado en posición " + posicion + ": " +
                        personas[posicion].nombre + ", " + personas[posicion].edad + " años.");
            } else {
